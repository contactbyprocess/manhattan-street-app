...
function initBannerCarousel(){
  const track=qs('#bannerTrack'), dots=qs('#bannerDots'); if(!track||!dots) return;
  const slides=[...track.querySelectorAll('img')];
  dots.innerHTML=slides.map((_,i)=>`<span class="dot ${i===0?'active':''}" data-i="${i}"></span>`).join('');
  let i=0; const width=()=>track.clientWidth;
  const go=n=>{i=(n+slides.length)%slides.length;track.scrollTo({left:i*width(),behavior:'smooth'});dots.querySelectorAll('.dot').forEach((d,di)=>d.classList.toggle('active',di===i));};
  const next=()=>go(i+1); let timer=setInterval(next,4000);
  dots.querySelectorAll('.dot').forEach(d=>d.addEventListener('click',()=>{go(+d.dataset.i);clearInterval(timer);timer=setInterval(next,4000);})); 
  window.addEventListener('resize',()=>{track.scrollLeft=i*width();});
}
